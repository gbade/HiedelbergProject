...
namespace HeidelbergCement.CaseStudies.Concurrency.Api.Test.Utilities;
  ...
  public class RequestBuilder
  {
        ...
        private static void ThrowIfError(HttpResponseMessage message)
        {
            ...
            {
                var responseContent = ParseErrorContent(message).Result;
                throw new HttpRequestException($"Could not make {message.RequestMessage?.Method.ToString() ?? "UNKNOWN METHOD"} / {message.RequestMessage?.RequestUri?.ToString() ?? "Unknown Uri"} Result: {responseContent.title} :: {responseContent.detail}");
            }
        }

        public static Task<ErrorResponse> ParseErrorContent(HttpResponseMessage message)
        {
            return message.DeserializeContent<ErrorResponse>();
        }
